...
struct Cache_Rule {
  const char* name;
  int len;
  max_count_t tokens[][MAX_L_STR];// i < 0 means key
};
...
max_count_t get_rule_count_at_length(int key, int rule, int loc, max_len_t l_str) {
    return Cache_G.defs[-key].rules[rule].tokens[loc][l_str];
}
void set_rule_count_at_length(int key, int rule, int loc, max_len_t l_str, max_count_t val) {
    Cache_G.defs[-key].rules[rule].tokens[loc][l_str] = val;
}
...
def tokens(rule, uninitialized_arr):
    return ', '.join([ uninitialized_arr for token in rule])


def print_def(k, i, rules, uninitialized_arr):
    key = k[1:-1]
    key_dec = '%(key)s = -%(i)d' % {'key':key, 'i': i}

    rule_def = '// new rule'.join(['''
//int* Cache_%(key)s_rule%(rulei)d_tokens[MAX_L_STR] = {%(tokens)s};

Cache_Rule Cache_%(key)s_rule%(rulei)d = {
    .name = "%(key)s %(rulei)d",
    .len = %(tokens_num)d,
    .tokens = {%(tokens)s} //Cache_%(key)s_rule%(rulei)d_tokens
};
''' % {'key':key, 'rulei':i, 'tokens_num': len(rule), 'tokens':tokens(rule, uninitialized_arr)} for i,rule in enumerate(rules)])
...
#"<digits>" : [["<digit>", "<digits>"],
#        ["<digit>"]],
"<digit>" : [[str(i)] for i in range(2)]
}

    max_l_str = 10;
    print(PRE % {'max_l' : 10, 'num_keys': len(g.keys()), 'key_len_cache_init': ', '.join(['UNINITIALIZED' for i in range(max_l_str*len(g.keys()))]) })
    compile_grammar(g, max_l_str)
    print(POST % {'max_l' : 10, 'num_keys': len(g.keys())})
